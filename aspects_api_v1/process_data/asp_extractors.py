...
class SentenceAspectExtractor():
    ...
    _my_stopword_additions = ["it's", "i'm", "i've", "time", "night", "try", "sure", "times", "way", "friends"]
    STOPWORDS = set(stopwords.words('english') + _my_stopword_additions)

    PUNCT_RE = re.compile("^[\".:;!?')(/]$")
    
    FORBIDDEN = {'great', 'good', 'friend', 'way', 'friends'}
...
#         tagged_sent = sentence.pos_tagged
        lemmatized_sent = sentence.lemmatized
        tree = SentenceAspectExtractor.CHUNKER.parse(lemmatized_sent)
        aspects = self.get_NPs(tree)

        # filter invalid aspects
        valid_aspects=[]
        for asp in aspects:
            valid_aspect = self.valid_aspect(asp)
            if valid_aspect!=None:
                valid_aspects.append(valid_aspect)
            else:
    ...
    def valid_aspect(self, aspect):
        ...
        OUTPUT: boolean
        """
        no_stops = [w for w in aspect if w not in SentenceAspectExtractor.STOPWORDS and not self.PUNCT_RE.match(w) and len(w)>3]
